...
Omega=cbind(rep(1,nresp*ntask),matrix(rtrun(mu=rep(.5,nresp*ntask),sigma=rep(.5,nresp*ntask),a=double(nresp*ntask),b=rep(100,ntask*nresp)),nc=1), runif(nresp*ntask), sample(c(0,1,2),replace=T,nresp*ntask,prob=double(3)+1/3))
ttheta=-2
...
keep(data, nresp, nalt, natt, nlvl, ntask, ndesn, 
     ...
     trates, tK, tgamma, tpsi, sure=T)
save.image("Simdata_PoisRaceTrackingV8_fixedPsi_smalltheta_balanced.RData")
rm(list = ls())
