...
stepb=.05 #stepsize for beta RW steps
stepdl=.02 #stepsize for delta and lambda RW steps
...
stepth=.15 #initial stepsize for theta RW
...
for(r in 1:R){
  accept=matrix(double(nresp*3),nr=nresp)
  ...
  acceptpropK=c(acceptpropK[2:accup],accept[,3])
