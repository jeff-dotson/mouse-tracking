...
setwd("/Users/rogerbailey/Dropbox/ESM Integrated Model/Code/Homogeneous Model")
...
#construct error changes for each individual (homoegeneous)
terr=matrix(1+c(0:(ntask-1))*talpha,nc=1)
...
tSigmaTr=diag(runif(nTr,.5,1))*mean(cbind(double(ntask)+1,terr-1)%*%tOmega)*.01
#compute tracking variables for each individual

Tr=array(0,dim=c(nresp,ntask,nTr))#true partworth values
for(i in 1:nresp){
  Tr[i,,]=cbind(double(ntask)+1,terr)%*%tOmega+t(t(chol(tSigmaTr))%*%matrix(rnorm(nTr*ntask),nr=nTr))
}
...
for(i in 1:nresp){
  ...
  utmat[,i]=constantut*rep(matrix((terr)^(-1)), each=nalt)
}
...
rm(altmat)
save.image("V3Simdata_HOMO_ESM_Int.RData")
