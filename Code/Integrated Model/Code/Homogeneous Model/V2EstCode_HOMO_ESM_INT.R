...
setwd("/Users/rogerbailey/Desktop/Projects/ESM/Integrated Model/Code/Homogeneous Model")

set.seed(77)

#Load data
load("V2Simdata_HOMO_ESM_Int.RData")
...
for(r in 1:R){
  ...
  for(i in 1:nresp){
    ...
    muvecold=cbind(double(ntask)+1,oldeng)%*%oldlambda
    muvecnew=cbind(double(ntask)+1,neweng)%*%oldlambda
    for(j in 1:ntask){
