...
ESMEstHeteroOmegaNoError=function(data,Prior,MCMC,sim,tvals){
  # FUNCTION OUTPUT:
  ...
  acceptpropd=double(accav)+.23 #proportion of delta draws accepted
  stepd=.8 #delta stepsize
  ...
  for(r in 1:R){
    ...
    for(resp in 1:nresp){
      ...
      oldgamma=track%*%oldomegamat[resp,]
      oldbeta=matrix(oldbetamat[resp,],nc=1)
      oldutmat=matrix(data[[resp]]$X%*%oldbeta/exp(rep(oldgamma+oldlambda,each=nalt)),nr=nalt)
    ...
    acceptpropo=c(acceptpropo[2:accav],mean(acceptmat[,2]))
  ...
  return(list( betadraw=betadraw, baccept=baccept ,bstepdraw=bstepdraw, Deltadraw=Deltadraw, Vbetadraw=Vbetadraw,
                deltadraw=deltadraw, daccept=daccept, dstepdraw=dstepdraw, 
                Omegadraw=Omegadraw, omegadraw=omegadraw, oaccept=oaccept, ostepdraw=ostepdraw, llikedraw=llikes))
}
