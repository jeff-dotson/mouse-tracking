...
if(sim==1){
  ...
  tscalecomp[,2:(ntask+1)]=matrix(colSums(matrix(matrix(rep(t(tomegas),ntask),nc=ntask)*Tr2,nr=nTr)),nc=ntask)
...
mean(tester)
#run the program
source("ESMEstHeteroOmegaNoError.R")
output=ESMEstHeteroOmegaNoError(data,Prior,MCMC,sim,Tvals)
